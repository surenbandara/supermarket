...
const SearchScreen = () => {
  ...
  const restaurants = restaurantsManager.getShopData();

  const searchAllShops = (searchText) => {
    console.log('bgggggg ', searchText)
    const data = []
    ...
    restaurants?.forEach((restaurant) => {
      if (regex.test(restaurant.name)) {data.push(restaurant)}
    })
    return data
  }
